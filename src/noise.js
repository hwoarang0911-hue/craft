// Deterministic noise toolkit: seeded PRNG, 2D/3D simplex, fbm helpers.
// Pure JS, safe for workers.
...
// fast deterministic 2D hash -> [0,1)
...
export function makeSimplex(rand) {
  ...
  function noise3D(xin, yin, zin) {
    ...
    return 32.69588470458984 * (n0 + n1 + n2 + n3);
  }
...
// ridged: |n| flipped -> sharp crests, in [0,1]
export function ridged2(noise, x, y, octaves = 4) {
  let amp = 0.5, freq = 1, sum = 0;
  for (let i = 0; i < octaves; i++) {
    sum += amp * (1 - Math.abs(noise.noise2D(x * freq, y * freq)));
    amp *= 0.5; freq *= 2.1;
  }
  return sum;
}
